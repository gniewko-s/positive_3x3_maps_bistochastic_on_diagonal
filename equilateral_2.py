...
* XC,YC - the 1-d numpy arrays of respectively x and y coordinates of the black shape arising from the CP condition.
	...
	#########################
	phi = np.linspace(0,2*np.pi,601) # 601
	r = CP(a,b,c)(phi)
	phi = np.pi/2 + phi
	XC = r*np.cos(phi)
	YC = r*np.sin(phi)
...
fig = plt.figure()
spec = gridspec.GridSpec(ncols=2, nrows=1, width_ratios=[3, 1], wspace=0, hspace=0)
...
ax1.arrow(0,0,1.3,0,head_width=2e-2);
ax1.arrow(0,0,0,1.3,head_width=2e-2);
phi = np.linspace(-2*np.pi/3,2*np.pi/3,201)
b = np.hstack(([2/3],2/3+np.cos(phi)/3+np.sin(phi)/3**.5))
c = np.hstack(([2/3],2/3+np.cos(phi)/3-np.sin(phi)/3**.5))
ax1.fill(b,c,color =(1,.9,.9))
ax1.fill([0,0,2/3,1],[0,1,2/3,0],color =(.9,.9,1))
b = np.linspace(0,.5,101); c = 2-b/2-(1+2.25*b**2)**.5
b,c = np.hstack((b,c[::-1],2/3)), np.hstack((c,b[::-1],2/3))
ax1.fill(b,c,color =(1,.9,1))
ax1.plot([0,1.2],[0,1.2],'--')
for x,y in [(0,1),(1,0),(1,1)]:
	ax1.plot(x,y, marker="o", markersize=4,markerfacecolor='black', markeredgecolor='black')
ax1.text(1.3, .02, 'b')
ax1.text(.02, 1.3, 'c')
...
K, = ax1.plot([bi],[ci],'r*')
ax1.set_title("",y=1.1,pad=-5)

fig.subplots_adjust(left=0, bottom=0.11)
ax0.set_title(f'$a = {ai:.2f}, b = {bi:.2f}, c = {ci:.2f}, \sqrt{{bc}}+a-2 = {(bi*ci)**.5+ai-2:.2f}$',y=1.1,pad=-5)
...
def redraw(s1,s2,s3):
	...
	E.set_xdata(XE)
	E.set_ydata(YE)
	K.set_visible(abs(s1-3) < 1e-6)
	ax1.set_title("" if abs(s1-3) < 1e-6 else "not optimal",y=1.1,pad=-5)
	K.set_xdata([b])
	K.set_ydata([c])
	ax0.set_title(f'$a = {a:.2f}, b = {b:.2f}, c = {c:.2f}, \sqrt{{bc}}+a-2 = {(b*c)**.5+a-2:.2f}$',y=1.1,pad=-5)
